...
const ProfilePage: React.FC<ProfilePageProps> = ({ user, favorites, onSignOut, onUpdateUser, showToast }) => {
  ...
  return (
    <div className="relative animate-in fade-in duration-400 fill-mode-forwards">
      ...
      <div className="max-w-[1200px] w-full mx-auto grid grid-cols-1 lg:grid-cols-[350px_1fr] gap-10 px-10 box-border animate-in slide-in-from-bottom-8 duration-800 ease-out fill-mode-forwards">
        
        {/* LEFT HERO SIDE */}
        <div className="bg-white/5 backdrop-blur-[30px] border border-white/[0.08] rounded-[30px] p-10 text-center h-fit sticky top-[120px] shadow-[0_25px_50px_-12px_rgba(0,0,0,0.5)]">
        ...
        <div className="space-y-8">
           <div className="bg-white/5 backdrop-blur-[30px] border border-white/[0.08] rounded-[30px] p-10 shadow-[0_25px_50px_-12px_rgba(0,0,0,0.5)]">
              <h3 className="font-bebas text-3xl text-accent mb-8 tracking-widest uppercase">Vault Settings</h3>
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <div className="flex gap-4">
                    <input 
                      type="text" 
                      className="flex-1 bg-bg border border-white/10 rounded-2xl px-6 py-4 text-text-custom font-dm text-base focus:border-accent outline-none transition-all shadow-inner"
                      value={name}
                      onChange={(e) => setName(e.target.value)}
                      placeholder="Enter your name"
                    />
                    <button 
                      className={`px-8 rounded-2xl font-bebas text-lg tracking-widest transition-all shadow-lg ${isUpdating ? 'bg-accent/50 cursor-not-allowed' : 'bg-accent text-bg hover:bg-[#f5c85a] active:scale-95'}`}
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-bg/50 border border-white/5 rounded-2xl p-6">
           ...
           <div className="bg-white/5 backdrop-blur-[30px] border border-white/[0.08] rounded-[30px] p-10 shadow-[0_25px_50px_-12px_rgba(0,0,0,0.5)]">
              <h3 className="font-bebas text-3xl text-accent mb-8 tracking-widest uppercase">Wishlist Statistics</h3>
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                <div className="bg-bg/50 border border-white/5 rounded-2xl p-6 text-center">
                   <div className="text-muted text-[0.6rem] font-bold uppercase tracking-[0.2em] mb-2">Movies</div>
                   <div className="text-[2rem] font-bebas text-accent">{favorites.movies?.length || 0}</div>
                </div>
                <div className="bg-bg/50 border border-white/5 rounded-2xl p-6 text-center">
                   <div className="text-muted text-[0.6rem] font-bold uppercase tracking-[0.2em] mb-2">Series</div>
                   <div className="text-[2rem] font-bebas text-accent">{favorites.series?.length || 0}</div>
                </div>
                <div className="bg-bg/50 border border-white/5 rounded-2xl p-6 text-center">
                   <div className="text-muted text-[0.6rem] font-bold uppercase tracking-[0.2em] mb-2">Games</div>
                   <div className="text-[2rem] font-bebas text-accent">{favorites.games?.length || 0}</div>
                </div>
              </div>
           </div>
        </div>
